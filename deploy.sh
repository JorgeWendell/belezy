...
APP_DIR="/var/www/belezy"
DOMAIN="belezy.adelbr.tech"
APP_PORT=3000
NODE_MAJOR=22
NODE_MAX_OLD_SPACE=1536

SERVICE_MEMORY_MAX="2048M"
SERVICE_CPU_QUOTA="200%"
SERVICE_TASKS_MAX="300"
SERVICE_NOFILE_LIMIT="65535"

POSTGRES_DB="belezy"
POSTGRES_USER="belezy_user"
...
if ! psql -h 127.0.0.1 -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -c "SELECT 1;" >/dev/null 2>&1; then
  echo "Falha ao validar conexão com PostgreSQL."
  exit 1
fi
unset PGPASSWORD
...
if [ ! -f package.json ]; then
  ...
  echo "Copie/clone o projeto para ${APP_DIR} antes de executar o deploy."
  exit 1
fi
...
required_envs=("DATABASE_URL" "BETTER_AUTH_SECRET" "BETTER_AUTH_URL" "NEXT_PUBLIC_BASE_URL")
...
[Unit]
Description=Belezy Next.js app
...
echo "Healthcheck: curl -I http://127.0.0.1:${APP_PORT}"
